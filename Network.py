...
class Network:    
        ...
        try:
            ...
            if length == self.nodes[0]:
                ...
                for l in range(1, self.N):
                    ...
                    self.Z[l] = np.matmul(self.weights[l], self.layers[l-1]) + self.biases[l]
                    self.layers[l] = self.act_func[self.functions[l]](self.Z[l])
                    
                    
                self.all_networks.append(self.layers)
            else:
    ...
    def __calc_all_deltas(self, deltas):
        ...
        for l in range(1, self.N - 1)[::-1]:
            ...
            deltas[l] = np.matmul(w, deltas[l+1]) * sigma_prime    
            
        self.deltas.append(deltas)
        
            
    def __calc_delta(self, inp):
        
        deltas= [0] + [0 * layer for layer in self.layers[1:]]
        deltas[-1] = (self.layers[-1] - inp) * self.back_func[self.functions[-1]](self.layers[-1])
        
        self.__calc_all_deltas(deltas)   
        
        
    def __calc_delta_softmax(self, inp):
        
        deltas= [0] + [0 * layer for layer in self.layers[1:]]
        deltas[-1] = self.softmax_prime(inp)
    ...
    def __update_weights(self):
        ...
        for l in range(1, len(self.weights)):
            ...
            del_w = 0 * self.weights[l]
            del_b = 0 * self.biases[l]
            
            for x in range(self.m):
            
                del_w = del_w + np.outer(self.deltas[x][l], self.all_networks[x][l-1])
                del_b = del_b + self.deltas[x][l]
                
            self.weights[l] = self.weights[l] - self.nue * del_w / self.m
            self.biases[l] = self.biases[l] - self.nue*del_b / self.m
    ...
    def train(self, inputs, targets):
        ...
        self.deltas = []
        self.m = len(inputs)
        if self.m == len(targets):
            
            for i in range(self.m):
                
                self.feed_forward(inputs[i])
                self.backpropogate(targets[i])
        ...
        try:
            ...
            if length == self.nodes[-1]:
                ...
                if self.functions[-1] == "softmax":
                    self.__calc_delta_softmax(inp)
                else:
                    self.__calc_delta(inp)
